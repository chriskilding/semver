...
@test "X is the major version..." {
    [[ $(semver awk --major <<< "1.9.0") = "1" ]]
}

@test "...Y is the minor version..." {
    [[ $(semver awk --minor <<< "1.9.0") = "9" ]]
}

@test "...and Z is the patch version." {
    [[ $(semver awk --patch <<< "1.9.0") = "0" ]]
}
...
@test "Example: 1.0.0-alpha" {
    [[ $(semver awk --prerelease <<< "1.0.0-alpha") = "alpha" ]]
}

@test "Example: 1.0.0-alpha.1" {
    [[ $(semver awk --prerelease <<< "1.0.0-alpha.1") = "alpha.1" ]]
}

@test "Example: 1.0.0-0.3.7" {
    [[ $(semver awk --prerelease <<< "1.0.0-0.3.7") = "0.3.7" ]]
}

@test "Example: 1.0.0-x.7.z.92" {
    [[ $(semver awk --prerelease <<< "1.0.0-x.7.z.92") = "x.7.z.92" ]]
}
...
@test "Example: 1.0.0-alpha+001" {
    [[ $(semver awk --build <<< "1.0.0-alpha+001") = "001" ]]
}

@test "Example: 1.0.0+20130313144700" {
    [[ $(semver awk --build <<< "1.0.0+20130313144700") = "20130313144700" ]]
}

@test "Example: 1.0.0-beta+exp.sha.5114f85" {
    [[ $(semver awk --build <<< "1.0.0-beta+exp.sha.5114f85") = "exp.sha.5114f85" ]]
}
